...
export const getSuperAdminEnabledType1 = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    try {
      ...
      name.setSSOAdmins(resp.data.admins);
      name.setSSOAdminCount(resp.data.admins.length);
      return resp.data;
    } catch (error) {
...
export const getSuperAdminEnabledType2 = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    try {
      ...
      // console.log("reduxResp",resp);
      name.setWalletAdmins(resp.data.admins);
      name.setWalletAdminCount(resp.data.admins.length);
      return resp.data;
    } catch (error) {
      console.log(error.response.data);
      if (error.response.data !== undefined) {
...
export const getSuperAdminDisabledType1 = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    try {
      ...
      name.setAdmins(resp.data.admins);
      name.setAdminCount(resp.data.admins.length);
      return resp.data;
    } catch (error) {
      console.log(error.response.data);
      if (error.response.data !== undefined) {
