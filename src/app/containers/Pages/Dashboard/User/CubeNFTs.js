...
function CubeNFTs(props) {
  ...
  let putOnSale = async (price, time, timeStamp) => {
    ...
    if (network !== "ropsten") {
      ...
      var myCubeContractInstance = await new web3.eth.Contract(
        cubeAbi,
        CubeAddress
      );
      let receipt = await myCubeContractInstance.methods
        .approve(address, cubeData.tokenId)
        .send({ from: accounts[0] }, (err, response) => {
          console.log("get transaction", err, response);
          if (err !== null) {
            console.log("err", err);
      ...
      axios.post(`auction/createsale`, SaleData).then(
        (res) => {
          ...
          setIsPuttingOnSale(false);
          let variant = "success";
          enqueueSnackbar("Successfully Allowed Exchange to Sale.", {
            variant,
          });
          getCubeNFTs();
        },
                  ...
                  {dropId !== "notdrop" ? (
                    ...
                    <div className="col-md-12 col-lg-6">
                      ...
                      {cubeData.salestatus || cubeData.check === "auction" ? (
                        <>
                          <Row>
                            <button className="btn-lg btn btn-block" disabled>
                              Allow Exchange to Sale
                            </button>
                          </Row>
                          <h4
                        ...
                        <>
                          <Row>
                            {isPuttingOnSale ? (
                              ...
                              <button
                                className="btn-lg btn btn-block"
                                onClick={(e) => {
                                  e.preventDefault();
                                  handleShow();
                                }}
