...
const AuctionNFT = (props) => {
    ...
    const [order, setOrderType] = useState("default");
    ...
    const sortData = (col) => {
        console.log(col);
        if (sortType === "Asc") {
          const sorted = [...bidDetail].sort((a, b) => a[col] - b[col]);
          setBidDetail(sorted);
          setSortType("Dsc");
          setOrderType('asc');
        }
        if (sortType === "Dsc") {
         const sorted = [...bidDetail].sort((a, b) => b[col] - a[col]);
         setBidDetail(sorted);
         setSortType("Asc");
         setOrderType('dsc');
    }
      ...
      <ThemeProvider theme={customTheme}>
        <div className="card-body px-0">
          <div className="row">
                            ...
                            <Col>
                                <form>
                                    ...
                                    <div className='row no-gutters align-items-center'>
                                        ...
                                        onClick = {(e) => {versionB === "v1-sso" ? (handleOpenModal(e)) : (handleBidSubmit(e))} }
                                        >
                                            Bid
                                        </button>
                                        </div>
                      ...
                      >
                        <thead>
                          <tr>
                            ...
                            <button
                                type="button"
                                onClick={() => sortData("bidAmount")}
                                style={{  backgroundColor:'transparent',outline: 'none', border: 'none',color:'white'}}
                            >
                                {order==='default' &&
                                   <><label> Bid  </label><FaSort /></>
                                }
                                {order==='asc' &&
                                   <><label> Bid  </label><FaSortUp style={{color:'lightblue'}}/></>
                                }
                                {order==='dsc' &&
                                   <><label> Bid  </label><FaSortDown style={{color:'lightblue'}}/></>
                                }
            ...
            <BidTxModal handleClose={handleCloseModal} open={modalOpen} handleBid = {handleBidSubmitSSO}  handlePay = {openTransak} dropData = {data} isOpen = {modalOpen} />
